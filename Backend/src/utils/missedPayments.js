...
export const checkMissedPayments = async () => {
  ...
  for (const sip of sips) {
    ...
    if (!payment) {
      ...
      const text = `Dear ${
        sip.user.fullname || sip.user.username
      },\n\nOur records indicate that you missed the SIP payment for "${
        sip.sipName
      }" scheduled on ${sip.nextPaymentDate.toDateString()}. Please take necessary action.\n\nBest regards,\nYour Investment App`;
